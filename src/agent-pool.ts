...
export function createAgentPool(deps: {
	...
	const evictIfNeeded = async () => {
		while (map.size > capacity) {
			const oldest = [...map.entries()].reduce((min, curr) =>
				curr[1].lastUsedAt < min[1].lastUsedAt ? curr : min,
			);
			if (!oldest) return;
	...
	return {
		...
		async closeAll() {
			...
			await Promise.allSettled(
				entries.map((entry) => disposeAgentSafely(entry.agent, log)),
			);
		},
	};
}
