...
export async function resolveAndPersistApiKey(deps: {
	...
	try {
		...
		const hasGet = (
			obj: unknown,
		): obj is { get: (...args: unknown[]) => unknown } =>
			typeof obj === "object" &&
			obj !== null &&
			"get" in obj &&
		...
		const hasSet = (
			obj: unknown,
		): obj is { set: (...args: unknown[]) => unknown } =>
			typeof obj === "object" &&
			obj !== null &&
			"set" in obj &&
		...
		const hasAuthenticate = (
			obj: unknown,
		): obj is { authenticate: (...args: unknown[]) => unknown } =>
			typeof obj === "object" &&
			obj !== null &&
			"authenticate" in obj &&
			typeof (obj as Record<string, unknown>).authenticate === "function";

		let savedAuth: unknown;
		if (hasGet(auth)) {
			savedAuth = await (
				auth as { get: (...args: unknown[]) => Promise<unknown> }
			).get
				.call(auth, { path: { id: "cursor" } })
				.catch(() => undefined);
		} else if (hasAuthenticate(auth)) {
			const authPromise = (
				auth as { authenticate: (...args: unknown[]) => Promise<unknown> }
			).authenticate.call(auth, { id: "cursor" });
			...
			savedAuth = await Promise.race([
				...
				.catch((err) => {
					if (log) {
						log.warn("cursor-provider: auth.authenticate failed or timed out", {
		...
		const result = await getOrRefreshToken(
			...
			async (tokens) => {
				if (hasSet(auth)) {
					await (auth as { set: (...args: unknown[]) => Promise<void> }).set
						.call(auth, {
							path: { id: "cursor" },
							body: {
								type: "oauth",
	...
	methods: [
		{
			...
			async authorize() {
				...
				return {
					...
					instructions:
						"Please complete login in your browser. NOTE: This session-based login may not work with some models. Manual API Key Entry is RECOMMENDED for stable use.",
					method: "auto",
					async callback() {
