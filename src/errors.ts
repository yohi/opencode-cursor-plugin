...
export function classifyError(
	...
	if (isNetwork) {
		return isPreDelivery
			? {
					retry: true,
					delayMs: 1000,
					reason: "NetworkError safe to retry pre-delivery",
				}
	...
	if (errorName === "ConfigurationError") {
		if (
			msgLower.includes("repository_required") ||
			msgLower.includes("repository is required")
		) {
			return noRetry(
				`ConfigurationError: ${CONFIG_ERROR_MESSAGES.REPOSITORY_REQUIRED}`,
			);
		}
		if (msgLower.includes("failed to verify existence of branch")) {
			return noRetry(
				`ConfigurationError: ${CONFIG_ERROR_MESSAGES.BRANCH_VERIFICATION_FAILED}`,
			);
		}
...
export const CONFIG_ERROR_MESSAGES = {
	REPOSITORY_REQUIRED:
		"Repository is required. Set CURSOR_REPO_URL or ensure git remote is configured.",
	BRANCH_VERIFICATION_FAILED:
		"Branch verification failed. Please ensure your GitHub account is connected at https://cursor.com/settings and has access to the repository. (Note: Bitbucket/GitLab may not be fully supported by Cursor's cloud agents; consider using a dummy GitHub URL as a workaround)",
} as const;
