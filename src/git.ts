	...
	try {
		...
		if (url) {
			...
			if (!url.startsWith("http://") && !url.startsWith("https://")) {
				if (log)
					log.debug("cursor-provider: ignored non-http repo url", { url });
				return undefined;
			}
			...
			try {
				...
				if (log)
					log.debug("cursor-provider: ignored invalid repo url format", {
						url,
					});
				return undefined;
			}
 ...
 */
export function resolveRepoInfo(log?: Logger): {
	url?: string;
	branch?: string;
} {
	return {
