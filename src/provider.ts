...
type ExtractedModelV2 = NonNullable<
	Awaited<ReturnType<ProviderModelsFn>>[string]
>;
...
type CursorModelsListFn = (opts: { apiKey: string }) => Promise<
	RawSDKModel[] | undefined | null
>;

async function listModelsWithTimeout(
	apiKey: string,
	log: Logger,
): Promise<FallbackModel[] | null> {
	const { Cursor } = await import("@cursor/sdk");
	const listModels = Cursor.models.list as CursorModelsListFn;
...
export function createProviderHook(deps: {
	...
	return {
		...
		async models(_provider: unknown, ctx: ProviderHookContext) {
			...
			const dynamicModels = apiKey
				? await listModelsWithTimeout(apiKey, log)
				: null;
...
async function getValidCursorModels(
	apiKey: string | undefined,
	log: Logger,
): Promise<Set<string>> {
	const allowedFromEnv = process.env.CURSOR_ALLOWED_MODELS;
	const envModels = allowedFromEnv
		? new Set(
				allowedFromEnv
					.split(",")
					.map((m) => m.trim())
...
function validateAndMapModel(
	...
	if (validSet.has("composer-2")) {
		log.warn("cursor-provider: mapping unknown model to composer-2", {
	...
	log.warn(
		"cursor-provider: proceeding with unknown model (no fallback found)",
		{ modelId },
	);
	return modelId;
}
...
async function resolveAgentAndMessage(deps: {
	...
	if (hit) {
		log.debug("cursor-provider: pool hit", {
	...
	log.debug("cursor-provider: pool miss", {
		prefixHash: translated.prefixHash.slice(0, 8),
	});
...
async function runDoStream(opts: {
	...
	if (
		!warnState.hasWarned() &&
		args.chatParams &&
		Object.keys(args.chatParams as Record<string, unknown>).length > 0
	) {
		warnState.markWarned();
		log.warn("cursor-provider: chat.params ignored", {
			paramKeys: Object.keys(args.chatParams as Record<string, unknown>),
		});
	}

	const translated = translate(args.prompt);
	const { agent, message, isHit } = await resolveAgentAndMessage({
		apiKey,
		modelId,
		translated,
		pool,
...
async function performAgentCreationAttempt(deps: {
	...
	{ agent: SDKAgent } | { error: unknown; canRetry: boolean; delay: number }
> {
	...
	try {
		...
		return {
			error: err,
			canRetry,
			delay: decision.delayMs * 2 ** (attempt - 1),
		};
	}
}
...
type CursorAgentFactory = {
	create: (opts: AgentCreateOpts) => Promise<SDKAgent>;
};

async function createAgentWithRetry(deps: {
	apiKey: string;
	modelId: string;
	log: Logger;
}): Promise<SDKAgent> {
	...
	const { log } = deps;
	const candidate = Agent as Partial<CursorAgentFactory>;

	if (typeof candidate.create !== "function") {
		throw new Error("Cursor SDK Agent.create is not available");
	}

	const cursorAgent: CursorAgentFactory = { create: candidate.create };
	...
	return Promise.reject(
		new Error("Agent creation failed after maximum retries"),
	);
}
