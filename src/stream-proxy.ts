...
import { classifyError, getErrorName, logError } from "./errors.js";
...
export function createStream(input: StreamProxyInput): {
	...
	const captureErrorType = (err: unknown) => {
		...
		lastErrorType = (
			validTypes.includes(rawName as StreamErrorType) ? rawName : "Error"
		) as StreamErrorType;
	};

	const safeEnqueue = (part: { type: string; [key: string]: unknown }) => {
		if (hasClosedStream) {
			log.debug("stream-proxy: enqueue after close ignored", {
				partType: part.type,
			});
			return;
		}
		...
		switch (update.type) {
			...
			case "tool-call-started": {
				const id =
					...
					(update.toolName as string | undefined) ??
					"unknown";
				...
				safeEnqueue({
					type: "text-delta",
					text: toolWarning(
						(update.toolName as string | undefined) ?? "unknown",
					),
				});
				break;
			}
	...
	const handleRunStatus = (result: { status: string }) => {
		...
		log.error("stream-proxy: run.status non-finished", {
			status: result.status,
		});
		safeEnqueue({
			type: "error",
			error: { message: `cursor: status=${result.status}` },
		});
		safeClose();
	};
			...
			void (async () => {
				try {
					...
					try {
						...
						const result = await (
							run as { wait: () => Promise<{ status: string }> }
						).wait();
						...
						if (
							...
							log.warn(
								"stream-proxy: UnknownAgentError; retrying with new agent",
							);
							...
							try {
								...
								const retryDecision = classifyError(retryErr, {
									phase: "create",
								});
								logError(log, retryErr, {
									phase: "create",
							...
							try {
								...
								const result = await (
									rerun as { wait: () => Promise<{ status: string }> }
								).wait();
								handleRunStatus(result);
							} catch (retryErr) {
								captureErrorType(retryErr);
						...
						while (attempt <= maxRetries) {
							...
							const backoffDelay = retryDecision.delayMs * 2 ** (attempt - 1);

							log.info(
								`stream-proxy: retrying (attempt ${attempt}/${maxRetries}) in ${backoffDelay}ms...`,
							);
