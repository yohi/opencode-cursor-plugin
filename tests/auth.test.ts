...
import { cursorAuthHook, resolveApiKey } from "../src/auth.js";

type AuthContext = {
	auth: { get: () => Promise<{ type: string; key: string } | undefined> };
};
...
describe("resolveApiKey", () => {
	...
	afterEach(() => {
		...
		else Reflect.deleteProperty(process.env, "CURSOR_API_KEY");
	});

	it("ctx.auth に api キーがあれば最優先で返す", async () => {
		process.env.CURSOR_API_KEY = "from-env";
		const ctx: AuthContext = {
			auth: { get: async () => ({ type: "api", key: "from-ctx" }) },
		};
	...
	it("ctx.auth が無ければ env をフォールバック", async () => {
		process.env.CURSOR_API_KEY = "from-env";
		const ctx: AuthContext = { auth: { get: async () => undefined } };
...
describe("cursorAuthHook", () => {
	it("methods に api タイプを含み、prompts は key 1 件", () => {
		expect(cursorAuthHook.methods.some((method) => method.type === "api")).toBe(
			true,
		);

		const apiMethod = cursorAuthHook.methods.find(
