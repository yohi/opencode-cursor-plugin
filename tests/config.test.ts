...
type PluginInput = Parameters<typeof CursorProviderPlugin>[0];
type ModelsCallCtx = {
	auth: { authenticate: (args: { id: string }) => Promise<unknown> };
};
...
describe("ensureCursorProviderConfig", () => {
	it("provider.cursor が無い設定に cursor provider 定義を追加する", () => {
		...
		expect(config.provider.cursor.models["composer-2"].name).toBe(
			"Composer 2 (initializing...)",
		);
	});
	...
	it("enabled_providers がある場合に cursor を自動追加する", () => {
		...
		expect(config.enabled_providers).toEqual(
			expect.arrayContaining(["openai", "google"]),
		);
	...
	it("プラグインの config hook から cursor provider 定義を追加する", async () => {
		...
		expect(config.provider.cursor.options.baseURL).toBe(
			"http://127.0.0.1:32125/v1",
		);
	});
});
...
describe("CursorProviderPlugin auth resolution", () => {
	it("auth.authenticate が成功した場合に認証情報を取得できる", async () => {
		const authenticate = vi
			.fn()
			.mockResolvedValue({ type: "api", key: "test-key" });
		const plugin = await CursorProviderPlugin({
			client: {
				app: { log: { info() {}, warn() {}, error() {}, debug() {} } },
				auth: { authenticate },
		...
		await plugin.provider?.models?.(
			{} as Config,
			{ auth: { authenticate } } as unknown as ModelsCallCtx,
		);
	...
	it("auth.authenticate がタイムアウトした場合に警告ログを出力しフォールバックする", async () => {
		...
		try {
			...
			const modelsPromise = plugin.provider?.models?.(
				{} as Config,
				{ auth: { authenticate } } as unknown as ModelsCallCtx,
			);
			...
			expect(warn).toHaveBeenCalledWith(
				expect.stringContaining("auth.authenticate failed or timed out"),
				expect.anything(),
			);
		} finally {
			vi.useRealTimers();
