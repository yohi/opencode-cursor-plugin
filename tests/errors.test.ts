...
describe("classifyError", () => {
	...
	it("NetworkError は create / pre-stream で retry:true (delay 1000ms)", () => {
		...
		expect(classifyError(err, { phase: "create" })).toMatchObject({
			retry: true,
			delayMs: 1000,
		});
		expect(classifyError(err, { phase: "pre-stream" })).toMatchObject({
			retry: true,
			delayMs: 1000,
		});
	});
	...
	it("メッセージに 'network error' や 'timeout' が含まれる場合も retry: true", () => {
		expect(
			classifyError(new Error("A network error occurred"), { phase: "create" }),
		).toMatchObject({ retry: true, delayMs: 1000 });
		expect(
			classifyError(new Error("request timed out"), { phase: "create" }),
		).toMatchObject({ retry: true, delayMs: 1000 });
	});

	it("RateLimitError は create / pre-stream で retry: true (delay 2000ms)", () => {
		expect(
			classifyError(new RateLimitError("rl"), { phase: "create" }),
		).toMatchObject({
			retry: true,
			delayMs: 2000,
		});
	...
	it("ResourceExhausted やメッセージに 'rate limit' が含まれる場合もレート制限として retry: true", () => {
		...
		expect(
			classifyError(new Error("Rate limit exceeded"), { phase: "create" }),
		).toMatchObject({ retry: true, delayMs: 2000 });
	});

	it("RateLimitError は in-stream / post-stream で retry: false (ストリーム重複防止)", () => {
		expect(
			classifyError(new RateLimitError("rl"), { phase: "in-stream" }).retry,
		).toBe(false);
		expect(
			classifyError(new RateLimitError("rl"), { phase: "post-stream" }).retry,
		).toBe(false);
	});

	it("ConfigurationError / IntegrationNotConnectedError / CursorSdkError は retry: false", () => {
		expect(
			classifyError(new ConfigurationError("cfg"), { phase: "pre-stream" })
				.retry,
		).toBe(false);
		expect(
			classifyError(
		...
		expect(
			classifyError(new CursorSdkError("sdk error"), { phase: "create" }).retry,
		).toBe(false);
	});
	...
	it("CursorSdkError のサブクラスも CursorSdkError として識別される (Issue 2)", () => {
		...
		expect(classifyError(err, { phase: "create" }).reason).toBe(
			"CursorSdkError",
		);
	});
	...
	it("予期せぬ例外は retry: false", () => {
		expect(classifyError(new Error("boom"), { phase: "create" }).retry).toBe(
			false,
		);
	});
});
...
describe("logError", () => {
	it("API キー文字列・prompt 本文をログに出さず、Allowlist にないキーも削除される", () => {
		...
		logError(log, new AuthenticationError("sk-very-secret-12345"), {
			phase: "create",
			secretKey: "hidden",
			model: "claude-3",
			prompt: "some-secret-prompt",
		});
