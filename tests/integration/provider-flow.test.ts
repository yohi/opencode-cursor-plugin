		...
		Cursor: {
			models: {
				list: vi
					.fn()
					.mockResolvedValue([
						{ id: "composer-2", name: "C2", contextWindow: 200_000 },
					]),
			},
		},
		Agent: {
...
type StreamAgent = Parameters<typeof createProviderHook>[0];
...
describe("integration: provider-flow", () => {
	const readAgents = (sdk: unknown): unknown[] => {
		const holder = sdk as { __agents?: unknown[] };
		return holder.__agents ?? [];
	};

	beforeEach(async () => {
		vi.clearAllMocks();
		const sdk = await import("@cursor/sdk");
		readAgents(sdk).length = 0;
	});
	...
	it("プールミス → put、続いてプールヒット → rekey", async () => {
		...
		const models = await hook.models?.("cursor" as never, {} as never);
		const model = models?.["composer-2"] as
			| {
					doStream: (args: {
						prompt: unknown[];
		...
		const sdk = await import("@cursor/sdk");
		await vi.waitFor(() => {
			expect(readAgents(sdk).length).toBe(1);
		});
	...
	it("LRU 退避: 容量超過で最古 agent が dispose される", async () => {
		const sdk = await import("@cursor/sdk");
		const beforeAgents = readAgents(sdk).length;
		...
		for (const prompt of ["U1", "U2", "U3"]) {
			const result = await model.doStream({
				prompt: [system("S"), user(prompt)],
			});
			await drain(result.stream);
		}

		await vi.waitFor(() => {
			const createdInTest = readAgents(sdk).slice(beforeAgents) as Array<
	...
	it("キャンセル後の次ターンで前ターンと別 agent が生成される", async () => {
		...
		let created = 0;
		originalCreate.mockImplementation(
			async (
				_options,
			): Promise<Awaited<ReturnType<typeof sdk.Agent.create>>> => {
				created += 1;
				if (created === 1) {
					return {
						send: vi.fn(
				...
				const agent = {
					...
					[Symbol.asyncDispose]: vi.fn().mockResolvedValue(undefined),
					close: vi.fn(),
				} as Awaited<ReturnType<typeof sdk.Agent.create>>;
				readAgents(sdk).push(agent);
				return agent;
			},
		);
		...
		try {
			...
			const first = await model.doStream({
				prompt: [system("S"), user("U1")],
				abortSignal: controller.signal,
			});
			controller.abort();
			await drain(first.stream);

			const before = readAgents(sdk).length;
			...
			await vi.waitFor(() => {
				expect(readAgents(sdk).length).toBe(before + 1);
			});
		} finally {
