...
describe("stream-proxy", () => {
  ...
  it("ToolCallStartedUpdate → 警告 text-delta は同 toolCallId で 1 回のみ", async () => {
    ...
    const { stream } = createStream({ agent, message: "m", log });
    const parts = await collect(stream);
    const warnings = parts.filter((part) => part.type === "text-delta" && /Cursor agent attempted/.test(part.text));
  ...
  it("PartialToolCallUpdate / ToolCallCompletedUpdate はドロップ", async () => {
    ...
    expect(parts.some((part) => part.type === "text-delta" && /foo/.test(part.text))).toBe(false);
  });
  ...
  it("AbortSignal でストリームがクローズされる", async () => {
    ...
    const { stream } = createStream({ agent, message: "m", log, abortSignal: controller.signal });
    const parts = await collect(stream);

    expect(parts.some((part) => part.type === "finish" && part.finishReason === "abort")).toBe(true);
  });
  ...
  it("createStream 開始時に既に abort 済みなら外部 abortSignal の listener を解除する", async () => {
    ...
    const { stream, done } = createStream({ agent, message: "m", log, abortSignal: controller.signal });
  ...
  it("UnknownAgentError + recreateAgent あり: 1 回再試行して成功する", async () => {
    ...
    expect(parts.some((part) => part.type === "text-delta" && part.text === "from-new")).toBe(true);
    await expect(done).resolves.toEqual({ finishReason: "stop" });
  });
